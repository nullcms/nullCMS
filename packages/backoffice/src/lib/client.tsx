...
const createApi = (apiClient: AxiosInstance) => ({
    ...
    collections: {
        ...
        getSchema: async (collection: string) => {
            const response = await apiClient.get(`/api/collections/${collection}/schema`);
    ...
    // Document operations
    documents: {
        getAll: async <T extends Document>(collection: string, options?: CollectionOptions) => {
            const params = new URLSearchParams();
            if (options?.skip !== undefined) params.append('skip', options.skip.toString());
            if (options?.limit !== undefined) params.append('limit', options.limit.toString());

            const response = await apiClient.get(`/api/collections/${collection}/documents`, { params });
            return response.data.documents as T[];
        },
        getById: async <T extends Document>(collection: string, id: string) => {
            const response = await apiClient.get(`/api/collections/${collection}/documents/${id}`);
            return response.data as T;
        },
        create: async <T extends Document>(collection: string, data: Omit<T, '_id'>) => {
            const response = await apiClient.post(`/api/collections/${collection}/documents`, data);
            return response.data as T;
        },
        update: async <T extends Document>(collection: string, id: string, data: Partial<Omit<T, '_id' | '_createdAt' | '_updatedAt'>>) => {
            const response = await apiClient.patch(`/api/collections/${collection}/documents/${id}`, data);
            return response.data as T;
        },
        delete: async (collection: string, id: string) => {
            const response = await apiClient.delete(`/api/collections/${collection}/documents/${id}`);
    ...
    singletons: {
        ...
        getSchema: async (singleton: string) => {
            const response = await apiClient.get(`/api/singletons/${singleton}/schema`);
            return response.data.schema;
        },
        ...
        update: async <T extends Document>(singleton: string, data: Partial<Omit<T, '_id' | '_createdAt' | '_updatedAt'>>) => {
            const response = await apiClient.patch(`/api/singletons/${singleton}`, data);
            return response.data as T;
        }
    }
});
...
// Document hooks
export function useDocuments<T extends Document>(collection: string, options?: CollectionOptions) {
    const { apiClient, baseURL } = useApi();
    const api = createApi(apiClient);
...
export function useDocument<T extends Document>(collection: string, id: string | null) {
    const { apiClient, baseURL } = useApi();
    const api = createApi(apiClient);

    return useQuery({
        queryKey: ['collections', baseURL, collection, 'documents', id],
        queryFn: () => api.documents.getById<T>(collection, id!),
...
export function useCreateDocument<T extends Document>(collection: string) {
    ...
    return useMutation({
        mutationFn: (data: Omit<T, '_id'>) => api.documents.create<T>(collection, data),
        onSuccess: async (e) => {
            toast("Document Created", {
                description: "Document has been created",
            });
            await queryClient.invalidateQueries({ queryKey: ['collections', collection, 'documents'] });
            await navigate({ to: '/collection/$name/$id', params: { name: collection, id: e._id } });
        },
        onError: (error) => {
...
export function useUpdateDocument<T extends Document>(collection: string) {
    ...
    return useMutation({
        mutationFn: ({ id, data }: { id: string, data: Partial<Omit<T, '_id' | '_createdAt' | '_updatedAt'>> }) =>
            api.documents.update<T>(collection, id, data),
        onSuccess: async (_, variables) => {
            await queryClient.invalidateQueries({ queryKey: ['collections', collection, 'documents', variables.id] });
            await queryClient.invalidateQueries({ queryKey: ['collections', collection, 'documents'] });
...
export function useDeleteDocument(collection: string) {
    ...
    return useMutation({
        mutationFn: (id: string) => api.documents.delete(collection, id),
        onSuccess: async (_, id) => {
            ...
            await queryClient.invalidateQueries({ queryKey: ['collections', collection, 'documents'] });
            await queryClient.invalidateQueries({ queryKey: ['collections', collection, 'documents', id] });
        },
        onError: (error) => {
...
export function useUpdateSingleton<T extends Document>(singleton: string) {
    ...
    return useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ['singletons', singleton] });
            toast("Document Saved", {
                description: "Document has been updated",
            });
        },
