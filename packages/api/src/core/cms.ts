...
import { StorageStrategyFactory, StorageConfig } from '../storage/factory';
import { CMSSchema, Document, CollectionSchema, SingletonSchema } from '@nullcms/shared';
...
export class CMS {
  ...
  constructor(storageConfig: StorageConfig, schema: CMSSchema) {
    this.storage = StorageStrategyFactory.createStrategy(storageConfig);
    this.schema = schema;
  ...
  async updateDocument<T extends Document>(
    ...
    await this.storage.update<T>(collection, { _id: id } as Partial<T>, data as Partial<T>);
    return await this.storage.findOne<T>(collection, { _id: id } as Partial<T>);
  }
  ...
  async deleteDocument<T extends Document>(
    ...
    const count = await this.storage.delete<T>(collection, { _id: id } as Partial<T>);
    return count > 0;
  }
