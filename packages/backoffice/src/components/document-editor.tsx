...
import type {
	ArrayField,
	BooleanField,
	CollectionSchema, // New field type
	DateField,
	Document,
	ExpandField,
	Field,
...
export function DocumentEditor({
	...
	const handleFieldChange = <T extends Field>(
		fieldId: string,
		value: FieldValue<T>,
	): void => {
		...
		if (fieldId.includes(".")) {
			...
			setValues((prev) => {
				...
				const currentTopValue = prev[topLevelField] as
				...
				if (parts.length === 2 && !Number.isNaN(Number(parts[1]))) {
					...
					const currentArray = Array.isArray(currentTopValue)
						? [...currentTopValue]
						: [];
				...
				for (let i = 1; i < parts.length - 1; i++) {
					...
					if (!current[part] || typeof current[part] !== "object") {
						...
						const nextIsNumber = !Number.isNaN(Number(parts[i + 1]));
	...
	function renderArrayItems<T extends Field>(
		fieldId: string,
		itemSchema: T,
		itemValues: Array<FieldValue<T>> | unknown,
	): ReactNode {
			...
			<>
				{values.map((itemValue, index) => (
					<div
						...
						className="border rounded p-2 pb-2 mb-2"
					>
						<Button
							type="button"
							variant="ghost"
							...
							onClick={() => {
								...
								handleFieldChange<ArrayField>(
									fieldId,
									newArr as FieldValue<ArrayField>,
								);
							}}
						>
							<Trash2 className="h-4 w-4" />
						...
						{renderFieldByType(`${fieldId}.${index}`, itemSchema, itemValue)}
					</div>
				))}
			</>
		);
	...
	function createDefaultValue(fieldSchema: Field): unknown {
		switch (fieldSchema.type) {
			...
			default:
				throw new Error(
					`Unsupported field type: ${(fieldSchema as { type: string }).type}`,
				);
		}
	}
	...
	const renderArrayField = (
		...
		return (
			<div className="grid gap-2">
				<div className="flex">
					<Label className="mr-auto" htmlFor={fieldId}>
						{field.label}
					</Label>
					<Button
						type="button"
						...
						onClick={() => {
							...
							setValues((prev) => {
								const currentArr = Array.isArray(prev[fieldId])
									? [...prev[fieldId]]
									: [];
								return {
									...prev,
		...
		<>
			<header className="border-b px-4 py-2 bg-background z-10">
				...
				<p className="text-sm text-muted-foreground">
					Last updated:{" "}
					{document?._updatedAt
						? formatRelative(document._updatedAt, new Date())
						: null}
				</p>
			</header>
