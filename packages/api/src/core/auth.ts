...
export class Auth {
    ...
    private async cleanupExpiredSessions(): Promise<void> {
        try {
            ...
            const expiredSessions = await this.storage.find<SessionDocument>(
                ...
                { expiresAt: { $lt: now } } as any // Using MongoDB-like query syntax
            );

            for (const session of expiredSessions) {
                await this.storage.delete(this.SESSION_COLLECTION, { _id: session._id });
            }
        } catch (error) {
    ...
    async createUser(
        ...
        if (username.length < 3 || username.length > 32 || username.trim() !== username) {
            throw new Error('Invalid username. Must be 3-32 characters and cannot start or end with whitespace.');
        }
        ...
        // Create user
        const newUser = await this.storage.insert<UserDocument>(this.USER_COLLECTION, {
            username,
            passwordHash,
            role,
            lastLogin: new Date()
     ...
     */
    async login(username: string, password: string): Promise<AuthResult & { token?: string }> {
        if (!this.isInitialized) {
            await this.initialize();
        }
    ...
    async validateSession(token: string): Promise<SessionValidationResult> {
        ...
        if (session.expiresAt <= renewalThreshold) {
            // Extend session
            const newExpiryDate = new Date(now.getTime() + (this.SESSION_EXPIRY_DAYS * 24 * 60 * 60 * 1000));
            await this.storage.update(
                this.SESSION_COLLECTION,
                { _id: sessionId } as Partial<SessionDocument>,
    ...
    async logoutAll(userId: string): Promise<number> {
        ...
        for (const session of sessions) {
            const deleted = await this.storage.delete(
                this.SESSION_COLLECTION,
                { _id: session._id } as Partial<SessionDocument>
            );
            count += deleted;
        }
    ...
    private async createSession(token: string, userId: string): Promise<void> {
        ...
        const now = new Date();
        const expiresAt = new Date(now.getTime() + (this.SESSION_EXPIRY_DAYS * 24 * 60 * 60 * 1000));
    ...
    private async verifyPassword(password: string, hash: string): Promise<boolean> {
        ...
        const hashedInput = encodeHexLowerCase(sha256(new TextEncoder().encode(password)));
    ...
    async getUsers(): Promise<Array<Omit<UserDocument, 'passwordHash'>>> {
        ...
        const users = await this.storage.find<UserDocument>(this.USER_COLLECTION, {});
     ...
     * Gets a user by ID
     */
    async getUserById(userId: string): Promise<Omit<UserDocument, 'passwordHash'> | null> {
        if (!this.isInitialized) {
            await this.initialize();
        }
