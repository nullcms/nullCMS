...
const DocumentInputSchema = DocumentSchema.omit({ _id: true, _createdAt: true, _updatedAt: true });
...
export function createRestApiRoutes(cms: CMS): OpenAPIHono<{ Variables: Variables }>  {
  ...
  api.openapi(
    ...
    async (c) => {
      try {
        ...
        if (!document) {
          return c.json({ error: `Document with ID '${id}' not found in collection '${collection}'` }, 404);
        }
  ...
  api.openapi(
    ...
    async (c: Context) => {
      try {
        ...
        if (!result) {
          return c.json({ error: `Singleton '${singleton}' not found or could not be updated` }, 404);
        }
  ...
  api.openapi(
      ...
      async (c) => {
        try {
          ...
          console.log(user);
          const document: any = await cms.auth.getUserById(user);
